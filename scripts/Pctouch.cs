    ...
    void Update()
    {
        if (Input.touchCount > 0)
        {
            if (Input.touches[0].phase == TouchPhase.Began)
            {
                ...
                {
                    hit.collider.gameObject.GetComponent<Pcdrag>().BeOnClicked();
                    activebutton = hit.collider.gameObject.GetComponent<Pcdrag>();
                    buttonactive = true;
                }
            }
        ...
        if (buttonactive)
        {
            activebutton.dragged(Camera.main.ScreenToWorldPoint(Input.mousePosition));
        }
#endif
    }
}
